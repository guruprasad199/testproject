...
defineFeature(feature, (test) => {
  ...
  test("User navigates to CountryCodeSelectorTable", ({
    ...
    when("I navigate to the CountryCodeSelectorTable", () => {
      instance = countryCodeSelectorBlock.instance() as CountryCodeSelectorTable;
    });
    ...
    when("should be render flatlist items", () => {
      ...
      const flatlistItem = shallow(flatlist.prop("renderItem")(renderItemData));
      const listItem = flatlistItem.findWhere(
    ...
    when("Should render header in flatlist", () => {
      const header = shallow(instance.renderHeader());
      const searchBar = header.findWhere(
        (node) => node.prop("testID") === "searchBar"
      );
      searchBar.simulate("changeText", "test");
    });
